      ...
      <nav className="bg-white/90 backdrop-blur-xl border-b border-slate-200/60 shadow-lg sticky top-0 z-50 transition-all duration-300">
        <div className="flex justify-between items-center max-w-7xl mx-auto px-4 sm:px-6 py-4">
          ...
          <div className="hidden lg:flex space-x-8">
            {navItems.map(([label, href]) => (
              <a key={href} href={href} className="text-slate-700 hover:text-blue-600 transition-all duration-300 font-medium hover:scale-105 relative group">
                {label}
        ...
        {isMenuOpen && (
          <div className="lg:hidden border-t border-slate-200 py-4 space-y-1">
            {navItems.map(([label, href]) => (
              <a key={href} href={href} onClick={() => setIsMenuOpen(false)} className="block rounded-lg px-3 py-3 text-slate-700 font-semibold hover:bg-blue-50 hover:text-blue-700">
                {label}
              </a>
            ))}
            <div className="flex gap-3 px-3 pt-3 sm:hidden">
              <Link href="/signin" className="flex-1 rounded-lg border border-blue-200 px-4 py-3 text-center font-semibold text-blue-700">Sign In</Link>
              <Link href="/signup" className="flex-1 rounded-lg bg-blue-600 px-4 py-3 text-center font-semibold text-white">Get Started</Link>
            </div>
          </div>
        )}
      </nav>
